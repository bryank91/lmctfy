...
define("confluence/page-message",["ajs","confluence/meta","window","confluence/api/browser","confluence-editor/editor/page-editor-message"],function(a,e,b,d,g){var c={},f=new d(b.navigator.userAgent);c._getQueryString=function(){return b.location.search};c.displayPageMessage=function(){if(!document.getElementById("confluence-ui")){var b=c._getQueryString(),d=e.get("editing-user");-1!==b.indexOf("editingFailed")?a.MessageHandler.flag({type:"info",title:"Unable to edit",body:a.format("Editing is unavailable right now. Try again in a few minutes.",
a.escapeHtml(d)),close:"manual"}):-1!==b.indexOf("userLimitReached")&&(g.handleMessage("collab.edit.user.limit.reached",{type:"warning",title:"You can\'t edit this page right now",message:"\u003cp\u003eThis page is so popular, you\'ve reached the maximum number of simultaneous editors.\u003c/p\u003e\u003cp\u003eTry again when it\'s not so busy.\u003c/p\u003e",close:"manual"}),a.Confluence.Analytics.publish("collab.edit.user.limit.reached",{browser:f.friendlyName()+"-"+f.version(),pageId:e.get("page-id"),editMode:"slow"}))}};return c});
require("confluence/module-exporter").safeRequire("confluence/page-message",function(a){require("ajs").toInit(a.displayPageMessage)});
}catch(e){WRMCB(e)};
;
try {
...
define("confluence/content-hover",["jquery","ajs"],function(d,e){var q={fadeTime:100,hideDelay:500,showDelay:700,loadDelay:50,width:300,mouseMoveThreshold:10,container:"body"};"undefined"===typeof e.followCallbacks&&(e.followCallbacks=[]);"undefined"===typeof e.favouriteCallbacks&&(e.favouriteCallbacks=[]);e.toInit(function(d){d(self).bind("hover-user.follow",function(d,m){d=0;for(var f=e.followCallbacks.length;d<f;d++)e.followCallbacks[d](m.username)})});return function(e,f,m,r,t){var g=d.extend(!1,
q,t),l,h,k=function(){var b=d("#content-hover-"+f);b.length?l=b.find(".contents"):(d(g.container).append(d('\x3cdiv id\x3d"content-hover-'+f+'" class\x3d"ajs-content-hover aui-box-shadow"\x3e\x3cdiv class\x3d"contents"\x3e\x3c/div\x3e\x3c/div\x3e')),b=d("#content-hover-"+f),l=b.find(".contents"),l.css("width",g.width+"px").mouseover(function(){var c=k()[0];clearTimeout(c.hideDelayTimer);b.unbind("mouseover")}).mouseout(function(){n()}),l.click(function(b){b.stopPropagation()}));return b},p=function(){var b=
k(),c=b[0];b.is(":visible")||(c.showTimer=setTimeout(function(){if(c.contentLoaded&&c.shouldShow){c.beingShown=!0;var a=d(window),e=h.x-3,f=h.y+15;e+g.width+30>a.width()?b.css({right:"20px",left:"auto"}):b.css({left:e+"px",right:"auto"});a=(window.pageYOffset||document.documentElement.scrollTop)+a.height();f+b.height()>a&&(f=a-b.height()-5,b.mouseover(function(){var a=k()[0];clearTimeout(a.hideDelayTimer)}).mouseout(function(){n()}));b.css({top:f+"px"});b.fadeIn(g.fadeTime,function(){c.beingShown=
!1})}},g.showDelay))},n=function(){var b=k(),c=b[0];c.beingShown=!1;c.shouldShow=!1;clearTimeout(c.hideDelayTimer);clearTimeout(c.showTimer);clearTimeout(c.loadTimer);c.contentLoading=!1;c.shouldLoadContent=!1;c.hideDelayTimer=setTimeout(function(){b.fadeOut(g.fadeTime)},g.hideDelay)};e.mousemove(function(b){h={x:b.pageX,y:b.pageY};var c=k(),a=c[0];a.beingShown||clearTimeout(a.showTimer);a.shouldShow=!0;if(!a.contentLoaded)if(a.contentLoading){if(a.shouldLoadContent&&(h.x-a.initialMousePosition.x)*
(h.x-a.initialMousePosition.x)+(h.y-a.initialMousePosition.y)*(h.y-a.initialMousePosition.y)>g.mouseMoveThreshold*g.mouseMoveThreshold){a.contentLoading=!1;a.shouldLoadContent=!1;clearTimeout(a.loadTimer);return}}else a.initialMousePosition=h,a.shouldLoadContent=!0,a.contentLoading=!0,a.loadTimer=setTimeout(function(){a.shouldLoadContent&&l.load(m,function(){a.contentLoaded=!0;a.contentLoading=!1;r.call(c,f);p()})},g.loadDelay);clearTimeout(a.hideDelayTimer);a.beingShown||p()}).mouseout(function(){n()});
d("body").click(function(){var b=k(),c=b[0];c.beingShown=!1;clearTimeout(c.hideDelayTimer);clearTimeout(c.showTimer);b.hide()})}});
}catch(e){WRMCB(e)};
;
try {
...
b+"': "+d.message)}};a.unescapeEntities=m;a.sessionCheck=function(a,d){var b=c.Meta.get("remote-user");if(b){var f=this.getContextPath()+h.REST.getBaseUrl()+"session/check/"+b,g=this;return e.getJSON(f).done(function(){c.debug("SESSION CHECK - OK - "+b)}).fail(function(){c.log("SESSION CHECK - FAIL - "+b);g.sessionFail(a,d)})}};a.sessionFail=function(a,d){if(!a){var b=e(".aui-dialog:visible");if(b.length)a=b.find(".dialog-panel-body").empty(),b.find(".aui-button").attr("disabled","disabled");else throw Error("sessionFail method really expected a visible dialog about now :/");
}d||(d=c.format("Your session has expired. Please {0}reload{1} the page to try again.",'\x3ca class\x3d"reload" href\x3d""\x3e',"\x3c/a\x3e"),a.on("click","a.reload",function(){g.location.reload()}));c.messages.warning(a,{body:d,closeable:!1,shadowed:!0})};a.Binder={placeFocus:p};a.hintManager=n;return a});require("confluence/module-exporter").exportModuleAsGlobal("confluence/root","Confluence",function(c){var e=require("ajs");e.toInit(function(){c.runBinderComponents()});e.Confluence=c});
}catch(e){WRMCB(e)};
;
try {
...
define("confluence/autocomplete-user",["jquery","ajs","document","confluence-rest/confluence-rest"],function(b,k,h,p){return function(g){g=g||h.body;var q=function(e){if(!e||!e.result)throw Error("Invalid JSON format");b.each(e.result,function(a,b){b.key=b.username||b.name;"group"===b.type&&(b.title=b.name,b.link=[],b.thumbnailLink={href:k.contextPath()+"/images/icons/avatar_group_48.png"})});var f=[];f.push(e.result);return f};b.each(["user","group","user-or-group"],function(e,f){b("input.autocomplete-"+
f+'[data-autocomplete-user-or-group-bound!\x3d"true"]',g).each(function(){var a=b(this).attr("data-autocomplete-user-or-group-bound","true").attr("autocomplete","off"),e=a.attr("data-max")||10,g=a.attr("data-alignment")||"left",l=a.attr("data-dropdown-target"),d=null,m=a.attr("data-target"),h=a.attr("data-show-unlicensed"),n=m&&b(m);l?d=b(l):(d=b("\x3cdiv\x3e\x3c/div\x3e"),a.after(d));a.attr("data-resize-to-input")&&(d.width(a.outerWidth()),d.addClass("resize-to-input"));d.addClass("aui-dd-parent autocomplete");
a.quicksearch(p.REST.getBaseUrl()+"search/"+f+".json",function(){a.trigger("open.autocomplete-user-or-group");"user"===f&&a.trigger("open.autocomplete-user")},{makeParams:function(a){return{"max-results":e,query:a,"show-unlicensed":h}},dropdownPlacement:function(a){d.append(a)},makeRestMatrixFromData:q,addDropdownData:function(b){if(!b.length){var c=a.attr("data-none-message");c&&b.push([{name:c,className:"no-results",href:"#"}])}return b},ajsDropDownOptions:{alignment:g,displayHandler:function(a){return a.restObj&&
a.restObj.username?a.name+" ("+k.escapeHtml(a.restObj.username)+")":a.name},selectionHandler:function(e,c){if(c.find(".search-for").length)a.trigger("selected.autocomplete-user-or-group",{searchFor:a.val()}),"user"===f&&a.trigger("selected.autocomplete-user",{searchFor:a.val()});else{if(!c.find(".no-results").length){c=b("span:eq(0)",c).data("properties");if(n){var d=c.restObj.title;c.restObj.username&&(d+=" ("+c.restObj.username+")");a.val(d);n.val(c.restObj.key)}else a.val(c.restObj.key);a.trigger("selected.autocomplete-user-or-group",
{content:c.restObj});"user"===f&&a.trigger("selected.autocomplete-user",{content:c.restObj})}this.hide();e.preventDefault()}}}})})})}});require("confluence/module-exporter").safeRequire("confluence/autocomplete-user",function(b){Confluence.Binder.autocompleteUserOrGroup=b});
}catch(e){WRMCB(e)};
;
try {
/* module-key = 'confluence.web.resources:core', location = 'includes/js/autocomplete-content.js' */
define("confluence/autocomplete-content",["jquery","document","ajs","confluence-rest/confluence-rest"],function(b,g,h,t){var u=function(a){if(!a||!a.group)throw Error("Invalid JSON format");for(var f=[],b=0,h=a.group.length;b<h;b++)f.push(a.group[b].result);return f},d=function(a,f,g,d){var c=b(a);if(!c.attr("data-autocomplete-content-bound")){"string"===typeof f&&(f=[f]);c.attr("data-autocomplete-content-bound","true").attr("autocomplete","off");var e=f.join(","),k=c.attr("data-max")||10;a=c.attr("data-alignment")||
"left";var m=c.attr("data-spacekey"),n=c.attr("data-none-message"),p=c.attr("data-search-link-message"),v=c.attr("data-template")||g,q=c.attr("data-target"),r=c.attr("data-dropdown-target"),l=null;r?l=b(r):(l=b("\x3cdiv\x3e\x3c/div\x3e"),c.after(l));l.addClass("aui-dd-parent autocomplete");c.focus(function(){m=c.attr("data-spacekey")});c.quicksearch(t.REST.getBaseUrl()+"search/name.json",null,{onShow:function(){c.trigger("open.autocomplete-content",{contentTypes:f})},makeParams:function(a){a={"max-results":k,
pageSize:k,type:e,query:a};m&&(a.spaceKey=m);return a},dropdownPlacement:function(a){l.append(a)},makeRestMatrixFromData:u,addDropdownData:function(a){a.length||n&&a.push([{name:n,className:"no-results",href:"#"}]);if(p){var b=h.escapeHtml(c.val()),b=h.format(p,b);a.push([{className:"search-for",name:b,href:"#"}])}return a},ajsDropDownOptions:{alignment:a,displayHandler:d,selectionHandler:function(a,d){h.debug("autocomplete-content: ajsDropDownOptions.selectionHandler");if(d.find(".search-for").length)c.trigger("selected.autocomplete-content",
{contentTypes:f,searchFor:c.val()});else{if(d.find(".no-results").length)h.debug("no results selected");else{var e=d.data("properties");c.val(h.template(v).fillHtml(e.restObj));if(q){var k=h.template(g).fillHtml(e.restObj);b(q).val(k)}c.trigger("selected.autocomplete-content",{contentTypes:f,content:e.restObj,selection:d})}this.hide();a.preventDefault()}}}})}},e={autocompleteSpace:function(a){a=a||g.body;b("input.autocomplete-space",a).each(function(){d(this,["spacedesc","personalspacedesc"],"{name}",
function(a){return a.name})})},autocompleteAttachment:function(a){a=a||g.body;b("input.autocomplete-attachment",a).each(function(){d(this,"attachment","{fileName}",function(a){var b=a.restObj&&a.restObj.fileName||a.name;a.restObj&&a.restObj.space&&a.restObj.space.title&&(b+=" ("+a.restObj.space.title+")");return b})})}},k=function(a){return a.restObj&&a.restObj.title&&h.escapeHtml(a.restObj.title)||a.name};e.autocompletePage=function(a){a=a||g.body;b("input.autocomplete-page",a).each(function(){d(this,
"page","{title}",k)})};e.autocompleteBlogpost=function(a){a=a||g.body;b("input.autocomplete-blogpost",a).each(function(){d(this,"blogpost","{title}",k)})};e.autocompleteConfluenceContent=function(a){a=a||g.body;b("input.autocomplete-confluence-content",a).each(function(){d(this,["page","blogpost"],"{title}",k)})};e.autocompleteSearch=function(a){a=a||g.body;b("input.autocomplete-search",a).each(function(){d(this,[],"{title}",k)})};return e});
